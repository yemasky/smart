$(function(){
	$.fn.simpleCY = function(options) {
		...
		function dateNoneParam(){
			var day = new Date();
			var today = new Array();
			today['year'] = day.getFullYear();//获得当前年份
			today['month'] = day.getMonth();//获得当前月份 + 1
			today['date'] = day.getDate();//获得当前天数
			today['hour'] = day.getHours();
			today['minute'] = day.getMinutes();
			today['second'] = day.getSeconds();
			today['week'] = day.getDay();//获得当前星期
