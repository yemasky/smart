...
class BookingServiceImpl implements \BaseServiceImpl {
	...
	public function bookHotelRoom(\HttpRequest $objRequest, \HttpResponse $objResponse): \SuccessService {
		...
		$arraySystemPrice  = $objRequest->validInput('system_price');
		$arrayChannelRoom  = $objRequest->validInput('room_num');
		if($channel_father_id === false || $arraySystemPrice === false || $arrayChannelRoom === false) {
		...
		if(isset($arraySystemPrice[$channel_id]) && isset($arrayChannelRoom[$channel_id])) {
			$arraySystem      = $arraySystemPrice[$channel_id];//筛选价格体系
			$arrayLayoutRoom  = $arrayChannelRoom[$channel_id];
			$arraySystemId    = [];
			$systemLayoutItem = [];
			//查找过滤有效的价格体系
			foreach($arraySystem as $layout_item_id => $system_id) {//组合价格体系
				if(!isset($arrayLayoutRoom[$layout_item_id])) continue;//没有设置此类型房间数量直接跳过
				if(empty($arrayLayoutRoom[$layout_item_id]['value'])) continue;//房间数量为0 也直接跳过
			...
			if(!empty($arraySystemId)) {
						...
						if(isset($arrayPriceLayout[$formulaData['system_father_id']])) {
							...
							foreach($formulaData['layout_item_id'] as $layout_item_id => $item_id) {
								if(isset($arrayPriceLayout[$formulaData['system_father_id']][$item_id])) {
									...
									for($i = 0; $i < $arrayLayoutRoom[$layout_item_id]['value']; $i++) {
				...
				return $objSuccess->setSuccessService(true, ErrorCodeConfig::$successCode['success'], '', $arrayPriceLayout);
			}
		}

		//返回参数错误
		return $objSuccess->setSuccessService(false, ErrorCodeConfig::$errorCode['parameter_error'], '', []);
	}
}